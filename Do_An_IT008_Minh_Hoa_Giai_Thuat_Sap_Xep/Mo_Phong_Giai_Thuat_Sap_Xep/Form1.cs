...
namespace Mo_Phong_Giai_Thuat_Sap_Xep
{
    public partial class Form1 : Form
    {
        ...
        async Task Di_Chuyen_pos_moi(Label lb, int tx, int ty, CancellationToken tk)
        {
            ...
            {
                await Tam_dung(tk);//Kiểm tra trạng thái dừng
                lb.Left += Math.Sign(tx - lb.Left) * van_toc_di_chuyen;//cập nhật tọa độ
                lb.Top += Math.Sign(ty - lb.Top) * van_toc_di_chuyen;//cập nhật tọa độ
                // Nhường quyền cho UI vẽ lại vị trí mới trong dotre ms
